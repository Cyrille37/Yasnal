...
var Yasnal = {
    config: {
		lib_uri: '',
		email_callback: '/email/callback.php'
	},
	auth_signature: null,
    ...
    authEmail: function (step) {
		...
		case 'emailSend':
			...
			{
				this.displayError('Cette adresse email semble invalide, vérifiez votre saisie');
				return ;
			}
			jQuery('#auth-error').hide();
			jQuery.post( this.config.lib_uri + this.config.email_callback, {action:'emailSend', email:email, csrf:csrf },
			function( jsonString ) {
