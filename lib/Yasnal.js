...
var Yasnal = {
	...
	$('.auth-google-button', form).click(function() {
	    var redirect_uri = Yasnal.config.lib_uri + Yasnal.config.google_connect_url ;
	...
	case 'codeConfirm':
	    var email = jQuery('#auth-form-email input:text[name=auth-email]')
		    .val();
	    var pincode = jQuery(
		    '#auth-form-email-code input:text[name=auth-emailCode]')
		    .val();
	    this.auth_callback({
		'provider' : 'email',
		'email' : email,
	...
	var auth_response = function(jsonString) {
	    var res = JSON.parse(jsonString);
	    if (res.status == 'ok') {
		// setTimeout('openVoteBox()', 1000);
		// jQuery.fancybox.close();
		if (Yasnal.config.auth_success_callback
			&& typeof (Yasnal.config.auth_success_callback) === "function") {
		    Yasnal.config.auth_success_callback();
		} else {
		    Yasnal
			    .displayError('Config error, auth_success_callback not defined');
		}
	    } else {
		if (res.status == 'error') {
		    Yasnal.displayError(res.message);
	...
	jQuery.post(this.config.lib_uri + this.config.auth_url, params,
		auth_response);
